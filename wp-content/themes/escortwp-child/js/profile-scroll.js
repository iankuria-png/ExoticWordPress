...
(function () {
    ...
    // ---------------------------------------------------------------
    // 4. MOBILE CONTACT: "Get in touch" accordion in sticky CTA
    // ---------------------------------------------------------------
    var mobileCta = document.querySelector('.profile-mobile-cta[data-mobile-contact]');
    if (mobileCta) {
        var mobileToggle = mobileCta.querySelector('[data-mobile-contact-toggle]');
        var mobilePanel = mobileCta.querySelector('[data-mobile-contact-panel]');

        if (mobileToggle && mobilePanel) {
            var setMobilePanelState = function (isOpen) {
                mobileToggle.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
                mobilePanel.hidden = !isOpen;
                mobileCta.classList.toggle('is-open', isOpen);
            };

            setMobilePanelState(false);

            mobileToggle.addEventListener('click', function () {
                setMobilePanelState(mobilePanel.hidden);
            });

            document.addEventListener('click', function (event) {
                if (mobilePanel.hidden) return;
                if (!mobileCta.contains(event.target)) {
                    setMobilePanelState(false);
                }
            });
    ...
    // ---------------------------------------------------------------
    // 5. GALLERY IMAGE LAZY-LOAD TRIGGER
