...
// Determine wizard steps based on context
$wizard_show_account = (!$escort_post_id && !$agencyid);
$wizard_steps = array();
if ($wizard_show_account) {
	$wizard_steps[] = array('key' => 'account', 'label' => __('Get Started', 'escortwp'), 'desc' => __('Create your account credentials', 'escortwp'));
}
$wizard_steps[] = array('key' => 'about', 'label' => __('About You', 'escortwp'), 'desc' => __('Tell us about yourself', 'escortwp'));
$wizard_steps[] = array('key' => 'location', 'label' => __('Location & Contact', 'escortwp'), 'desc' => __('Where can clients find you?', 'escortwp'));
$wizard_steps[] = array('key' => 'services', 'label' => __('Services & Rates', 'escortwp'), 'desc' => __('What do you offer?', 'escortwp'));
$wizard_total = count($wizard_steps);
$wizard_has_errors = (isset($err) && $err && isset($_POST['action']) && $_POST['action'] == 'register') ? '1' : '0';
?>
...
<form action="<?php echo $form_url; ?>" method="post" class="form-styling register-form" id="register_form" data-has-errors="<?php echo $wizard_has_errors; ?>">
    ...
    <!-- Wizard Progress Bar -->
    <div class="wizard-progress" role="navigation" aria-label="<?php esc_attr_e('Registration progress', 'escortwp'); ?>">
        <div class="wizard-progress__track">
            <div class="wizard-progress__fill" aria-hidden="true"></div>
        </div>
        <ol class="wizard-progress__steps">
            <?php foreach ($wizard_steps as $i => $step) : ?>
            <li class="wizard-progress__step<?php echo $i === 0 ? ' wizard-progress__step--active' : ''; ?>" data-target="<?php echo $i; ?>">
                <span class="wizard-progress__dot" aria-hidden="true"><?php echo $i + 1; ?></span>
                <span class="wizard-progress__label"><?php echo esc_html($step['label']); ?></span>
            </li>
            <?php endforeach; ?>
        </ol>
    </div>

    <div class="sr-only" aria-live="polite" id="wizard-announce"></div>

    <?php
    // Track which wizard step index we're on for output
    $wizard_step_index = 0;
    ?>

    <!-- ============================================
         STEP 1: GET STARTED (Account + Name)
         Only shown for new self-registrations
         ============================================ -->
    <?php if ($wizard_show_account) : ?>
    <div class="wizard-step wizard-step--active" data-step="<?php echo $wizard_step_index; ?>" data-step-key="account" role="region" aria-label="<?php echo esc_attr($wizard_steps[$wizard_step_index]['label']); ?>">
        <div class="wizard-step__header">
            <h2 class="wizard-step__title"><?php echo esc_html($wizard_steps[$wizard_step_index]['label']); ?></h2>
            <p class="wizard-step__desc"><?php echo esc_html($wizard_steps[$wizard_step_index]['desc']); ?></p>
        </div>
    ...
    </div><!-- /wizard-step account -->
    <?php $wizard_step_index++; ?>
    <?php endif; ?>

    <!-- ============================================
         STEP 2: ABOUT YOU (Identity + Physical + Lifestyle)
         ============================================ -->
    <div class="wizard-step<?php echo ($wizard_step_index === 0) ? ' wizard-step--active' : ''; ?>" data-step="<?php echo $wizard_step_index; ?>" data-step-key="about" role="region" aria-label="<?php echo esc_attr($wizard_steps[$wizard_step_index]['label']); ?>" <?php echo ($wizard_step_index > 0) ? 'hidden' : ''; ?>>
        <div class="wizard-step__header">
            <h2 class="wizard-step__title"><?php echo esc_html($wizard_steps[$wizard_step_index]['label']); ?></h2>
            <p class="wizard-step__desc"><?php echo esc_html($wizard_steps[$wizard_step_index]['desc']); ?></p>
        </div>

        <?php // In agency/edit mode (no Step 1), Name field goes here
        if (!$wizard_show_account) : ?>
        <?php if(!$agencyid) { ?>
        <div class="form-label">
            <label for="youremail"><?php _e('Your Email','escortwp'); ?><i>*</i></label>
    ...
    </div><!-- /wizard-step about -->
    <?php $wizard_step_index++; ?>

    <!-- ============================================
         STEP 3: LOCATION & CONTACT
         ============================================ -->
    <div class="wizard-step" data-step="<?php echo $wizard_step_index; ?>" data-step-key="location" role="region" aria-label="<?php echo esc_attr($wizard_steps[$wizard_step_index]['label']); ?>" hidden>
        <div class="wizard-step__header">
            <h2 class="wizard-step__title"><?php echo esc_html($wizard_steps[$wizard_step_index]['label']); ?></h2>
            <p class="wizard-step__desc"><?php echo esc_html($wizard_steps[$wizard_step_index]['desc']); ?></p>
        </div>
    ...
    </div><!-- /wizard-step location -->
    <?php $wizard_step_index++; ?>

    <!-- ============================================
         STEP 4: SERVICES & RATES
         ============================================ -->
    <div class="wizard-step" data-step="<?php echo $wizard_step_index; ?>" data-step-key="services" role="region" aria-label="<?php echo esc_attr($wizard_steps[$wizard_step_index]['label']); ?>" hidden>
        <div class="wizard-step__header">
            <h2 class="wizard-step__title"><?php echo esc_html($wizard_steps[$wizard_step_index]['label']); ?></h2>
            <p class="wizard-step__desc"><?php echo esc_html($wizard_steps[$wizard_step_index]['desc']); ?></p>
        </div>
    ...
    </div><!-- /wizard-step services -->

    <!-- Wizard Navigation -->
    <div class="wizard-nav">
        <button type="button" class="wizard-nav__btn wizard-nav__btn--prev" disabled aria-label="<?php esc_attr_e('Previous step', 'escortwp'); ?>">
            ...
            <?php _e('Back', 'escortwp'); ?>
        </button>
        <div class="wizard-nav__counter">
            <span class="wizard-nav__current">1</span> / <span class="wizard-nav__total"><?php echo $wizard_total; ?></span>
        </div>
        <button type="button" class="wizard-nav__btn wizard-nav__btn--next" aria-label="<?php esc_attr_e('Next step', 'escortwp'); ?>">
            <?php _e('Continue', 'escortwp'); ?>
